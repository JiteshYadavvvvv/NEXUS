...
import { Building2, Plus, Trash2, Users, FileText, UserCog, X, Loader2, Check } from 'lucide-react';
...
export default function ManageClubs() {
    ...
    const startFacultyEdit = (club) => {
        setFacultyEditId(club._id);
        setFacultyValue(club.faculty || '');
    };

    const handleSaveFaculty = async (clubId) => {
        setSavingFaculty(true);
        try {
            const res = await axios.put(
                `${API}/api/superadmin/change-faculty`,
                { clubId, facultyId: facultyValue.trim() },
                { withCredentials: true }
            );
            if (res.data?.success) {
                toast.success('Faculty updated');
                setClubs((prev) => prev.map((c) => (c._id === clubId ? { ...c, faculty: facultyValue.trim() } : c)));
                setFacultyEditId(null);
            } else {
                toast.error(res.data?.message || 'Failed to update faculty');
            }
        } catch (err) {
            ...
            <div className="flex items-start justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Create, configure, and remove institute clubs</p>
                    </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {clubs.map((club) => (
                        <div key={club._id} className="rounded-2xl border border-gray-200 bg-white p-5 hover:shadow-md transition-shadow">
                            ...
                            {/* Faculty */}
                            <div className="mb-4">
                                <div className="text-[11px] uppercase tracking-wider text-gray-400 mb-1">Faculty In-charge</div>
                                {facultyEditId === club._id ? (
                                    <div className="flex items-center gap-2">
                                        <input
                                            value={facultyValue}
                                            onChange={(e) => setFacultyValue(e.target.value)}
                                            placeholder="Faculty name / email"
                                            className="flex-1 rounded-lg border border-gray-300 px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                        />
                                        <button
                                            onClick={() => handleSaveFaculty(club._id)}
                                            disabled={savingFaculty}
                                            className="rounded-lg bg-blue-600 p-2 text-white hover:bg-blue-700 disabled:opacity-50"
                                        >
                                            {savingFaculty ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                                        </button>
                                        <button onClick={() => setFacultyEditId(null)} className="rounded-lg bg-gray-100 p-2 text-gray-500 hover:bg-gray-200">
                                            <X className="h-4 w-4" />
                                        </button>
                                    </div>
                                ) : (
                                    <button
                                        onClick={() => startFacultyEdit(club)}
                                        className="w-full text-left rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 transition-colors"
                                    >
                                        {club.faculty || <span className="text-gray-400">Click to assign faculty</span>}
                                    </button>
                                )}
                            </div>
